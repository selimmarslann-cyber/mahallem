...
export default function ProfileScreen() {
  ...
  const userOrders = currentUser
    ...
    : []
  const vendorOrders = currentUser?.role === 'vendor'
    ? orders.filter((o) => {
      ...
      >
        ...
        <View style={styles.profileHeader}>
          ...
          <Text style={styles.userName}>
            {currentUser.name} ({currentUser.role === 'vendor' ? 'Esnaf' : 'Kullanıcı'})
          </Text>
          <TouchableOpacity
                ...
                return (
                  <View key={order.id} style={styles.orderCard}>
                    <View style={styles.orderHeader}>
                      <Text style={styles.orderVendorName}>{vendor?.businessName || 'İşletme'}</Text>
                    ...
                    <Text style={styles.orderDate}>
                      {new Date(order.createdAt).toLocaleDateString('tr-TR')} - {order.scheduledFor}
                    </Text>
                  </View>
                )
              })
          ...
          <View style={styles.section}>
            ...
            {vendorOrders.length > 0 ? (
              vendorOrders.map((order) => (
                <View key={order.id} style={styles.orderCard}>
                  <View style={styles.orderHeader}>
                    <Text style={styles.orderVendorName}>Sipariş #{order.id.slice(-6)}</Text>
                    <Text style={styles.orderStatus}>{order.status}</Text>
                  </View>
                  <Text style={styles.orderDescription} numberOfLines={2}>
                    {order.description}
        ...
        <View style={styles.section}>
          <View style={styles.referralCard}>
            ...
            <View style={styles.referralCodeContainer}>
              <Text style={styles.referralCode}>MAH{currentUser.id.slice(-5).toUpperCase()}</Text>
              <TouchableOpacity
            ...
            >
              ...
              <Text style={styles.referralLinkButtonText}>Sıfır Yatırımla Ortak Ol</Text>
        ...
        <View style={styles.section}>
          ...
          >
            <Ionicons name="storefront-outline" size={24} color={colors.cardBg} />
