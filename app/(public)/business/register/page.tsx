...
import { findProductByName, getProductSuggestions, type ProductData } from '@/lib/data/product-database'
...
export default function BusinessRegisterPage() {
  ...
  const handleProductNameChange = (name: string) => {
    ...
    if (name.trim().length > 2) {
      ...
      const suggestions = getProductSuggestions(formData.category || 'RESTORAN', name)
      ...
      if (product) {
        ...
        if (editingItem) {
          setMenuItems(menuItems.map(i => 
            i.id === editingItem.id 
              ? { ...i, imageUrl: product.imageUrl, ingredients: product.ingredients }
              : i
          ))
        }
      }
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">
            Dükkanınızı haritada işaretleyin, bilgilerinizi girin ve menünüzü ekleyin
          </p>
        </div>
        ...
        <div className="mb-6 flex items-center gap-4">
          <div className={cn(
            ...
            step === 'location' ? "text-primary font-semibold" : step === 'info' || step === 'menu' ? "text-primary" : "text-gray-400"
          )}>
            <div className={cn(
              "w-8 h-8 rounded-full flex items-center justify-center",
              step === 'location' ? "bg-primary text-white" : step === 'info' || step === 'menu' ? "bg-primary/20 text-primary" : "bg-gray-200"
            )}>
              1
            </div>
            <span>Konum Seç</span>
          ...
          <div className={cn(
            "flex items-center gap-2",
            step === 'info' ? "text-primary font-semibold" : step === 'menu' ? "text-primary" : "text-gray-400"
          )}>
            <div className={cn(
              "w-8 h-8 rounded-full flex items-center justify-center",
              step === 'info' ? "bg-primary text-white" : step === 'menu' ? "bg-primary/20 text-primary" : "bg-gray-200"
            )}>
              2
            </div>
            <span>Bilgiler</span>
            ...
            {step === 'location' && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Haritada dükkanınızın konumunu işaretleyin. Haritaya tıklayarak konum seçebilirsiniz.
                  </CardDescription>
                </CardHeader>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="text-sm text-blue-900">
                        ...
                        <ol className="list-decimal list-inside space-y-1 text-blue-800">
                          ...
                          <li>Seçilen konum turuncu bir işaret ile gösterilecek</li>
                  ...
                  {selectedLocation && (
                    <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                      <p className="text-sm text-green-900">
                        <strong>Seçilen Konum:</strong> {selectedLocation.lat.toFixed(6)}, {selectedLocation.lng.toFixed(6)}
                      </p>
                    </div>
                  )}
                  
                  <Button onClick={handleNextFromLocation} className="w-full" disabled={!selectedLocation}>
            ...
            {step === 'menu' && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    İşletmenizin menüsünü veya hizmetlerini ekleyin (Yemeksepeti tarzı)
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {/* Add/Edit Item Form */}
                  <div className="border rounded-lg p-4 space-y-3 bg-gray-50">
                    <h4 className="font-semibold">{editingItem ? 'Ürünü Düzenle' : 'Yeni Ürün Ekle'}</h4>
                      ...
                      <Input
                        ...
                        onFocus={() => {
                          if (newItem.name.length > 2) {
                            const suggestions = getProductSuggestions(formData.category || 'RESTORAN', newItem.name)
                            setProductSuggestions(suggestions)
                            setShowSuggestions(suggestions.length > 0)
                          }
                        }}
                  ...
                  <div className="space-y-2">
                    ...
                    {menuItems.length === 0 ? (
                      <div className="border-2 border-dashed rounded-lg p-8 text-center text-gray-500">
                        ...
                        <p className="text-sm mt-1">Yukarıdaki formdan ürün ekleyebilirsiniz</p>
                      </div>
                    ) : (
                      <div className="space-y-2">
                        {menuItems.map((item) => (
                          <div key={item.id} className="border rounded-lg p-3 flex items-start gap-3 hover:bg-gray-50">
                            {item.imageUrl && (
                              <img 
                                src={item.imageUrl} 
                                alt={item.name}
                            ...
                            <div className="flex-1">
                              ...
                              {item.ingredients && item.ingredients.length > 0 && (
                                <div className="mt-2">
                                  <div className="text-xs text-gray-500 mb-1">Malzemeler:</div>
                              ...
                              <div className="text-primary font-semibold mt-2">{item.price.toFixed(2)} ₺</div>
                            </div>
                            <div className="flex gap-2">
                              <Button
                                variant="outline"
