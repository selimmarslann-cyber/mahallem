...
export default function BusinessJobsPage() {
  ...
  const getStatusText = (status: string) => {
    const statusMap: Record<string, { text: string; variant: any; icon: any }> = {
      PENDING_CONFIRMATION: { text: 'Yeni İstek', variant: 'secondary', icon: Clock },
      ACCEPTED: { text: 'Kabul Edildi', variant: 'default', icon: CheckCircle2 },
      IN_PROGRESS: { text: 'Devam Ediyor', variant: 'default', icon: Clock },
      COMPLETED: { text: 'Tamamlandı', variant: 'default', icon: CheckCircle2 },
      CANCELLED_BY_CUSTOMER: { text: 'Müşteri İptal Etti', variant: 'destructive', icon: XCircle },
      CANCELLED_BY_PROVIDER: { text: 'İptal Edildi', variant: 'destructive', icon: XCircle },
    }
    return statusMap[status] || { text: status, variant: 'outline', icon: Clock }
  }
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Siparişlerinizi yönetin ve müşterilerinize hizmet verin</p>
            </div>
        ...
        {activeTab === 'new' && newOrders.length > 0 && (
          <Alert className="mb-6 border-orange-200 bg-orange-50">
            ...
            <AlertDescription className="text-orange-900">
              <strong>Önemli:</strong> 2 saat içinde cevap vermezseniz, hesabınız otomatik olarak offline'a alınır.
            </AlertDescription>
          </Alert>
        )}
