...
export function getTransporter() {
  ...
  if (!process.env.MAIL_HOST) {
    throw new Error('MAIL_HOST environment variable is required. Please set it in .env file.')
  }
  if (!process.env.MAIL_PORT) {
    throw new Error('MAIL_PORT environment variable is required. Please set it in .env file.')
  }
...
export async function sendMail(
  ...
  // Config kontrolü
  if (!process.env.MAIL_USER || !process.env.MAIL_PASS) {
    const error = new Error('Email configuration is missing. Please set MAIL_USER and MAIL_PASS environment variables.')
  ...
  if (!fromEmail) {
    throw new Error('MAIL_FROM or MAIL_USER environment variable is required. Please set it in .env file.')
  }
  ...
  try {
    ...
    if (error.code === 'EAUTH') {
      errorMessage = 'E-posta kimlik doğrulama hatası. MAIL_USER ve MAIL_PASS kontrol edin.'
    } else if (error.code === 'ECONNECTION') {
      errorMessage = 'SMTP sunucusuna bağlanılamadı. MAIL_HOST ve MAIL_PORT kontrol edin.'
