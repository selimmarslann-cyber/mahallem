...
import { Calendar, MapPin, DollarSign, CheckCircle2, ArrowLeft } from 'lucide-react'
    ...
    try {
      const response = await fetch('/api/instant-jobs/create', {
        ...
        body: JSON.stringify({
          ...
          scheduledAt: formData.when === 'custom' && formData.customDateTime
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-2xl mx-auto px-4">
        <Link href="/jobs" className="inline-flex items-center gap-2 text-sm text-gray-600 mb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {step === 2 && (
                <div className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <Button
                      type="button"
                      variant={formData.when === 'today' ? 'default' : 'outline'}
                      className="w-full justify-start"
                      onClick={() => setFormData({ ...formData, when: 'today' })}
                    >
                      <Calendar className="w-4 h-4 mr-2" />
                      Bugün
                    </Button>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          setFormData({ ...formData, customDateTime: e.target.value, when: 'custom' })
                        }}
                        className="mt-2"
                      />
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          setFormData({ ...formData, requiresSkills: checked === true })
                        }
                      />
                      <Label htmlFor="requiresSkills" className="font-normal cursor-pointer">
              ...
              <div className="flex justify-between pt-4">
                ...
                <Button type="submit" className="ml-auto" disabled={loading}>
                  {step === 4 ? (loading ? 'Oluşturuluyor...' : 'İş Oluştur') : 'Devam'}
                </Button>
              </div>
            </form>
          </CardContent>
