 ...
 */
export async function cancelOrderByProvider(orderId: string, businessId: string) {
  const order = await prisma.order.findUnique({
    where: { id: orderId },
    include: { business: true },
  })
  ...
  return prisma.$transaction(async (tx) => {
    ...
    await tx.order.update({
      ...
      data: {
        status: 'CANCELLED_BY_PROVIDER',
        paymentStatus: order.paymentStatus === 'CAPTURED' ? 'REFUNDED' : order.paymentStatus,
      },
    })
...
export async function updateOrderStatus(
  ...
  if (status === 'COMPLETED' && !['ACCEPTED', 'IN_PROGRESS'].includes(order.status)) {
 ...
 * İşletme siparişleri
 */
export async function getBusinessOrders(businessId: string, status?: OrderStatus) {
