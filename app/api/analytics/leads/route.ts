...
export async function GET(request: NextRequest) {
  try {
    ...
    const startDate = searchParams.get('startDate') || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString() // Son 30 gün
    const endDate = searchParams.get('endDate') || new Date().toISOString()

    // 1. Toplam Lead Satın Alma İstatistikleri
    const { data: totalPurchases, error: purchasesError } = await supabaseAdmin
      .from('lead_purchases')
      .select('id, lead_fee_tl, created_at, contact_made_at, job_status, refund_processed')
    ...
    const totalRevenue = totalPurchases?.reduce((sum, p) => sum + (p.lead_fee_tl || 0), 0) || 0
    const totalListingsCount = totalListings?.length || 0

    // İletişim kurulma oranı
    const contactedLeads = totalPurchases?.filter((p) => p.contact_made_at).length || 0
    const contactRate = totalLeads > 0 ? (contactedLeads / totalLeads) * 100 : 0

    // İş tamamlanma oranı
    const completedLeads = totalPurchases?.filter((p) => p.job_status === 'completed').length || 0
    const completionRate = totalLeads > 0 ? (completedLeads / totalLeads) * 100 : 0

    // İade oranı
    const processedRefunds = totalRefunds?.filter((r) => r.refund_status === 'processed').length || 0
    const refundRate = totalLeads > 0 ? (processedRefunds / totalLeads) * 100 : 0
    ...
    // İptal oranı
    const cancelledListings = totalListings?.filter((l) => l.cancelled_at).length || 0
    const cancellationRate = totalListingsCount > 0 ? (cancelledListings / totalListingsCount) * 100 : 0
