...
import { CheckCircle2, Zap, Star, Sparkles, Shield, TrendingUp, TrendingDown } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      {showTrend && trendDirection && (
        <>
          {trendDirection === 'up' ? (
            <TrendingUp className={cn(iconSizeClasses[size], 'text-green-600')} />
          ) : (
            <TrendingDown className={cn(iconSizeClasses[size], 'text-red-600')} />
          )}
        </>
      )}
    </div>
