...
export default function VendorProfileScreen() {
  ...
  const renderStars = (rating: number) => {
    ...
    for (let i = 0; i < emptyStars; i++) {
      stars.push(
        <Ionicons key={`empty-${i}`} name="star-outline" size={20} color="#E0E0E0" />
      )
    }
    return stars
  }
      ...
      >
        ...
        <View style={styles.vendorInfo}>
          ...
          <View style={styles.ratingContainer}>
            ...
            <Text style={styles.ratingText}>{(vendor?.rating || 0).toFixed(1)}</Text>
            {vendor.reviewCount && (
              <Text style={styles.reviewCount}>({vendor.reviewCount} değerlendirme)</Text>
            )}
          </View>
          ...
          {vendorMenu.length > 0 ? (
            vendorMenu.map((item) => (
              <TouchableOpacity
                ...
                onPress={() => {
                  navigation.navigate('OrderFlow' as never, {
                    vendorId: vendor?.id,
                    serviceId: item.id,
                  } as never)
                }}
        ...
        <View style={styles.actionButtons}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              navigation.navigate('OrderFlow' as never, { vendorId: vendor.id } as never)
            }}
          >
