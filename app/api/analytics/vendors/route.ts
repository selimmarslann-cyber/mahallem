...
export async function GET(request: NextRequest) {
  try {
    ...
    // Usta bilgilerini al
    const vendorIds = [...new Set(purchases?.map((p) => p.vendor_id) || [])]
    ...
    purchases?.forEach((purchase) => {
      ...
      if (purchase.contact_made_at) {
        ...
        const responseTimeHours = (contactTime - purchaseTime) / (1000 * 60 * 60)
    ...
    Object.values(vendorPerformance).forEach((perf: any) => {
      if (perf.responseTimes.length > 0) {
        perf.avgResponseTime = Math.round(
          (perf.responseTimes.reduce((sum: number, time: number) => sum + time, 0) /
      ...
      perf.contactRate =
        perf.totalLeads > 0 ? Math.round((perf.contactedLeads / perf.totalLeads) * 10000) / 100 : 0
      perf.completionRate =
        perf.totalLeads > 0 ? Math.round((perf.completedLeads / perf.totalLeads) * 10000) / 100 : 0
      perf.cancellationRate =
        perf.totalLeads > 0 ? Math.round((perf.cancelledLeads / perf.totalLeads) * 10000) / 100 : 0
      perf.refundRate =
        perf.totalLeads > 0 ? Math.round((perf.refundedLeads / perf.totalLeads) * 10000) / 100 : 0

      // Net spend (harcama - iadeler)
      perf.netSpent = perf.totalSpent - perf.totalRefundAmount

      // ROI hesapla (eğer completed leads varsa)
      perf.roi = perf.completedLeads > 0 ? Math.round((perf.completedLeads / perf.netSpent) * 100) / 100 : 0
        ...
        summary: {
          ...
          avgLeadsPerVendor:
            Object.keys(vendorPerformance).length > 0
              ? Math.round(((purchases?.length || 0) / Object.keys(vendorPerformance).length) * 100) / 100
              : 0,
        },
      },
      { status: 200 }
