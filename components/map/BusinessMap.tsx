...
export default function BusinessMap({ businesses, onBusinessClick, onMapMove }: BusinessMapProps) {
  ...
  const updateMarkers = useCallback(() => {
    ...
    const onlineBusinesses = businesses.filter((b) => b.onlineStatus === 'ONLINE')
    const otherBusinesses = businesses.filter((b) => b.onlineStatus !== 'ONLINE')
          ...
          <Card>
            <CardContent className="p-4">
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <Badge
                  variant={selectedBusiness.onlineStatus === 'ONLINE' ? 'success' : 'secondary'}
                >
                  {selectedBusiness.onlineStatus === 'ONLINE' ? 'Aktif' : 'Offline'}
                </Badge>
              </div>
              <div className="flex items-center gap-1 mb-2">
                <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                <span className="text-sm">
                  {selectedBusiness.avgRating.toFixed(1)} ({selectedBusiness.reviewCount} değerlendirme)
                </span>
              </div>
              <p className="text-sm text-gray-600 mb-3">{selectedBusiness.addressText}</p>
