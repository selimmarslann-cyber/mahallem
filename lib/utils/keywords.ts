 ...
 */
export function getKeywordSuggestions(query: string, limit: number = 10): string[] {
  ...
  const contains = allKeywords.filter(
    (kw) => kw.includes(lowerQuery) && !kw.startsWith(lowerQuery) && kw !== lowerQuery
  )

  // Öncelik sırası: tam eşleşme > başlangıç > içinde geçen
  const suggestions = [...exactMatches, ...startsWith.slice(0, limit), ...contains.slice(0, limit - exactMatches.length - startsWith.length)]
