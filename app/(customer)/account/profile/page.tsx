...
export default function AccountProfilePage() {
  ...
  const loadUser = async () => {
    try {
      ...
      if (!res.ok) {
        router.push(`/auth/required?page=Profil&redirect=${encodeURIComponent('/account/profile')}`)
        return
      }
      ...
      // Load referral link
      if (data.user.id) {
        const referralRes = await fetch('/api/referral/overview', { credentials: 'include' })
        if (referralRes.ok) {
          const referralData = await referralRes.json()
          setReferralLink(referralData.referralLink || `https://mahallem.com/ref/${data.user.id}`)
        }
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#F5F5F7] pt-24 pb-24 md:pb-0">
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        <div>
          ...
          <p className="text-slate-600">Kişisel bilgilerinizi yönetin ve ayarlarınızı güncelleyin</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card className="border-2 border-slate-200">
            <CardHeader>
              ...
              <CardDescription>
                Hangi sektörde çalışıyorsunuz? Bu alanda hangi işlere bakarsınız?
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                      ...
                      return (
                        <Badge
                          ...
                          className={`cursor-pointer transition-all ${
                            isSelected ? 'bg-[#FF6000] text-white border-[#FF6000]' : ''
                          }`}
                        ...
                        onCheckedChange={async (checked) => {
                          ...
                          try {
                            const res = await fetch('/api/user/settings', {
                              method: 'PATCH',
                              headers: { 'Content-Type': 'application/json' },
                              body: JSON.stringify({ instantJobNotifications: checked }),
                              credentials: 'include',
                            })
                            if (!res.ok) {
                              setInstantJobNotifications(!checked)
