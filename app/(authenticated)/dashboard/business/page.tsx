...
export default function BusinessDashboardPage() {
  ...
  const activeOrders = orders.filter(
    (o) => ['ACCEPTED', 'IN_PROGRESS'].includes(o.status)
  )
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-800 mt-1">
                    Hesabınız {new Date(business.bannedUntil).toLocaleDateString('tr-TR', {
                      day: 'numeric',
                      month: 'long',
                      year: 'numeric',
                    })} tarihine kadar sistem tarafından geçici olarak durdurulmuştur.
                    Üst üste 3 iptal sebebiyle 7 günlük ban uygulanmıştır.
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3">
                  <Badge variant={getStatusBadgeVariant()} className="text-base px-3 py-1">
                    {getStatusText()}
                  </Badge>
                  {isBanned && (
                    <Badge variant="destructive">
                      Banlı ({new Date(business.bannedUntil).toLocaleDateString('tr-TR')})
                    </Badge>
                  )}
                </div>
              </div>
              <Button
                ...
                disabled={isBanned}
                variant={business.onlineStatus === 'ONLINE' ? 'destructive' : 'default'}
                size="lg"
              >
                <Power className="w-4 h-4 mr-2" />
                {business.onlineStatus === 'ONLINE' ? 'Offline Yap' : 'Online Yap'}
              </Button>
            </div>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <p className="text-2xl font-bold">{business.avgRating.toFixed(1)}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{business.consecutiveCancellations}</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              {pendingOrders.length === 0 ? (
                ...
                <div className="space-y-2">
                  {pendingOrders.map((order) => (
                    <OrderCard key={order.id} order={order} onUpdate={() => loadOrders(business.id)} />
                  ))}
                </div>
              )}
            </div>
