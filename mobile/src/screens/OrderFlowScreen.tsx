...
export default function OrderFlowScreen() {
  ...
  const [selectedDate, setSelectedDate] = useState<'urgent' | 'today' | 'custom' | null>(null)
        ...
        {step === 1 && (
          <View style={styles.stepContainer}>
            ...
            <Text style={styles.stepDescription}>
              {vendor?.businessName || vendor?.name} işletmesinden hangi hizmeti almak istiyorsunuz?
            </Text>
            {vendorMenu.length > 0 ? (
              vendorMenu.map((item) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.menuItemCard,
                    selectedServiceId === item.id && styles.menuItemCardSelected,
                  ]}
                  onPress={() => setSelectedServiceId(item.id)}
                >
                  <Text style={styles.menuItemName}>{item.name}</Text>
                  <Text style={styles.menuItemPrice}>{item.price.toLocaleString('tr-TR')} ₺</Text>
                </TouchableOpacity>
              ))
            ) : (
          ...
          <View style={styles.stepContainer}>
            ...
            >
              <Ionicons
                name="flash"
                size={24}
                color={selectedDate === 'urgent' ? colors.cardBg : colors.primary}
              />
              <View style={styles.dateOptionContent}>
                ...
                <Text
                  style={[
                    ...
                    selectedDate === 'urgent' && styles.dateOptionDescriptionActive,
                  ]}
                >
                  En kısa sürede
                </Text>
            ...
            >
              <Ionicons
                name="calendar"
                size={24}
                color={selectedDate === 'today' ? colors.cardBg : colors.primary}
              />
              <View style={styles.dateOptionContent}>
                <Text
                  style={[
                ...
                <Text
                  style={[
                    styles.dateOptionDescription,
                    selectedDate === 'today' && styles.dateOptionDescriptionActive,
                  ]}
                >
                  Bugün içinde
                </Text>
            ...
            >
              <Ionicons
                ...
                size={24}
                color={selectedDate === 'custom' ? colors.cardBg : colors.primary}
              />
              <View style={styles.dateOptionContent}>
                <Text
                  style={[
                ...
                <TextInput
                  ...
                  placeholderTextColor={selectedDate === 'custom' ? colors.cardBg : colors.textMuted}
                  value={customDate}
          ...
          <View style={styles.stepContainer}>
            <View style={styles.summaryCard}>
              ...
              <View style={styles.summaryRow}>
                ...
                <Text style={styles.summaryValue}>{vendor?.businessName || vendor?.name}</Text>
              </View>
              {selectedServiceId && (
                <View style={styles.summaryRow}>
                  <Text style={styles.summaryLabel}>Hizmet:</Text>
                  <Text style={styles.summaryValue}>
                    {vendorMenu.find((m) => m.id === selectedServiceId)?.name || 'Özel hizmet'}
                  </Text>
                </View>
              )}
              <View style={styles.summaryRow}>
              ...
              <View style={styles.summaryRow}>
                ...
                <Text style={styles.summaryValue}>{description || 'Açıklama yok'}</Text>
              </View>
            </View>
          </View>
        )}
