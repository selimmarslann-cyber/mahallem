 ...
 * 
 * AI (Groq) kullanarak kullanıcının hizmet açıklamasından
 ...
 */
async function getActiveCategories(): Promise<Array<{ id: string; name: string; slug: string }>> {
 ...
 * Groq hata verirse veya sonuç gelmezse kullanılır
 */
...
/**
 * Groq ile kategori sınıflandırması yap
 * 
...
{ "slugs": ["slug-1", "slug-2"] }`
  ...
  try {
    ...
    try {
      ...
      console.warn('Groq response JSON parse hatası:', parseError)
      // Fallback'e geç
    }
  } catch (groqError) {
  ...
  // Eğer Groq'dan slug gelmediyse, fallback kullan
  if (!selectedSlug) {
  ...
  return {
    ...
    confidence: 'high', // Groq kullanıldıysa high, fallback ise medium olabilir
  }
}
