/**
 * Domain Types - Mahallem Mobile
 *
 * Bu dosya, mobil uygulama için domain tiplerini içerir.
 * Web versiyonu (lib/types/domain.ts) ile aynı tipleri kullanır
 * ki FAZ 2'de backend'e bağlanırken kolay eşleşsin.
 *
 * FAZ 2'de bu tipler backend API response'larıyla doldurulacak.
 */
...
export interface Order {
  ...
  businessId: string;
  totalAmount: number;
  vendorAmount: number; // FAZ 3: Vendor pay
  ...
  items: OrderItem[];
  business?: Business | null; // Business relation
  review?: any | null; // Review relation
  createdAt: string;
  updatedAt: string;
}
