...
import { JobFormData, ProviderServicePreferences } from '../types/service-categories';
...
export function matchesJob(
  ...
  const hasMainCategory = providerPreferences.mainCategories.includes(job.mainCategoryId);
  ...
  const hasSubService = providerPreferences.subServices.includes(job.subServiceId);
