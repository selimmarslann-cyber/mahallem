...
export default function EarnPage() {
  ...
  const [onlyNoSkills, setOnlyNoSkills] = useState(false)
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (userLocation) {
          ...
          // Sort by distance
          jobsWithDistance.sort((a, b) => (a.distanceKm || 999) - (b.distanceKm || 999))
  ...
  useEffect(() => {
    ...
    if (userLocation) {
      filtered = filtered.filter((job) => {
        const distance = job.distanceKm || haversineDistanceKm(
          userLocation,
          { lat: job.locationLat, lng: job.locationLng }
        )
    ...
    // Skills filter
    if (onlyNoSkills) {
      filtered = filtered.filter((job) => !job.requiresSkills)
    }

    setFilteredJobs(filtered)
  }, [instantJobs, searchQuery, maxDistance, onlyNoSkills, statusFilter, userLocation])
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8">
          <Card className="bg-brand-50 border-brand-100">
            <CardContent className="p-4 md:p-5">
              <p className="text-sm text-brand-700 font-medium mb-1">Bu hafta toplam</p>
              <p className="text-2xl md:text-3xl font-bold text-brand-900">{openJobsCount} iş</p>
            </CardContent>
          </Card>
          
          <Card className="bg-emerald-50 border-emerald-100">
            <CardContent className="p-4 md:p-5">
              <p className="text-sm text-emerald-700 font-medium mb-1">Ortalama iş süresi</p>
              <p className="text-2xl md:text-3xl font-bold text-emerald-900">35 dk</p>
            </CardContent>
          </Card>
          
          <Card className="bg-sky-50 border-sky-100">
            <CardContent className="p-4 md:p-5">
              <p className="text-sm text-sky-700 font-medium mb-1">Ortalama kazanç</p>
              <p className="text-2xl md:text-3xl font-bold text-sky-900">120 ₺/iş</p>
            </CardContent>
          </Card>
        </div>
          ...
          <Card className="border-2 border-emerald-200 hover:shadow-lg hover:-translate-y-0.5 transition-all">
            ...
            <CardContent>
              ...
              <div className="text-xs text-textSecondary bg-emerald-50 p-3 rounded-xl border border-emerald-100">
                💡 <strong>İpucu:</strong> Ek gelir kazanmak için profilini doldur ve bildirimleri aç.
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="rounded-full bg-surface shadow-sm border border-borderSoft px-4 py-2 flex items-center justify-between gap-3 mb-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              <Switch
                id="onlyNoSkills"
                checked={onlyNoSkills}
                onCheckedChange={setOnlyNoSkills}
              />
              <Label htmlFor="onlyNoSkills" className="text-sm cursor-pointer">
                Sadece vasıf gerektirmeyenler
              </Label>
            </div>
          </div>
        </div>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-textSecondary mb-4">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>
                              {job.scheduledAt
                                ? new Date(job.scheduledAt).toLocaleDateString('tr-TR', {
                                    day: 'numeric',
                                    month: 'short',
                                    hour: '2-digit',
                                    minute: '2-digit',
