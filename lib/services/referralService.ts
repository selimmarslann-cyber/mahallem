...
const LEVEL_1_SHARE_RATE = 0.20 // %20
const LEVEL_2_SHARE_RATE = 0.10 // %10
 ...
 */
export async function distributeReferralRewards(orderId: string): Promise<void> {
  ...
  await prisma.$transaction(async (tx) => {
    for (const relation of referralRelations) {
      const shareRate = relation.level === 1 ? LEVEL_1_SHARE_RATE : LEVEL_2_SHARE_RATE
...
export async function getReferralOverview(userId: string) {
  ...
  return {
    ...
    level1Earnings,
    level2Earnings,
    currentBalance: (await prisma.user.findUnique({
      where: { id: userId },
