...
import { Calendar, TrendingUp, XCircle, AlertTriangle, CheckCircle2 } from 'lucide-react'
...
export default function BusinessStatusPage() {
  ...
  const calculateStats = (ordersData: any[]) => {
    ...
    const last30DaysCancellations = ordersData.filter((o) => {
      ...
      return (
        orderDate >= monthAgo &&
        (o.status === 'CANCELLED_BY_PROVIDER' || o.status === 'CANCELLED_BY_CUSTOMER')
      )
    }).length
        ...
        {isAutoOffline && (
          <Alert className="bg-yellow-50 border-yellow-200">
            ...
            <AlertDescription className="text-yellow-800">
              Son gelen işlere cevap vermediğiniz veya mesai saati dışında olduğunuz için
              hesabınız otomatik offline'a alınmıştır.
              <Button
                variant="outline"
                size="sm"
        ...
        <div className="grid grid-cols-1 gap-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Bugünkü İş Sayın</CardTitle>
              <Calendar className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{stats?.todayCount || 0}</div>
              <p className="text-xs text-muted-foreground mt-1">Bugün planlanan işler</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Bu Hafta Kazancın</CardTitle>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">Son 7 gündeki tamamlanan işler</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Son 30 Günde İptal Sayın</CardTitle>
              <XCircle className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{stats?.last30DaysCancellations || 0}</div>
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        <p className="font-semibold text-sm">
                          {order.scheduledAt
                            ? new Date(order.scheduledAt).toLocaleTimeString('tr-TR', {
                                hour: '2-digit',
                                minute: '2-digit',
                              })
                            : 'Saat belirtilmemiş'}
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-gray-500">{order.business?.category}</p>
                      </div>
                    </div>
                  </div>
                ))}
