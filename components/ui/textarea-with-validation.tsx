...
export function TextareaWithValidation({
  ...
  const validation = useFieldValidation(props.value || '', rules, validateOnChange)

  return (
    <div className="space-y-2">
      {label && (
        <Label htmlFor={props.id} className={cn(validation.showError && 'text-red-500')}>
          {label}
        </Label>
      )}
      <Textarea
        ...
        aria-describedby={validation.showError ? `${props.id}-error` : undefined}
      />
