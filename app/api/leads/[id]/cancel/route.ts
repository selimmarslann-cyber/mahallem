...
export async function POST(
  ...
  try {
    ...
    const { data: leadPurchase, error: purchaseError } = await supabaseAdmin
      .from('lead_purchases')
      .select('id, listing_id, vendor_id, lead_fee_tl, refund_requested, refund_processed, job_status')
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'Lead başarıyla iptal edildi. İade talebi oluşturuldu ve admin onayı bekleniyor.',
      },
      { status: 200 }
    )
  } catch (error: any) {
