...
export async function GET(
  ...
  try {
    ...
    if (canViewContact && customer) {
      ...
      response.contactMessage = 'İletişim bilgilerini görmek için bu ilan için ödeme yapmanız gerekmektedir.'
    }
    ...
    if (userId) {
      ...
      if (user?.role === 'VENDOR' && !canViewContact) {
        ...
        response.wallet = {
          ...
          sufficient: wallet ? Number(wallet.balance) >= listing.lead_fee_tl : false,
        }
      }
    }
