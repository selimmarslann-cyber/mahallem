...
export async function GET(
  ...
  try {
    ...
    if (job.status === 'PENDING') {
          ...
          OR: [
            ...
            job.isOther
              ...
              : {
                  ...
                  mainCategories: { has: job.mainCategoryId },
                  subServices: job.subServiceId ? { has: job.subServiceId } : undefined,
                },
          ],
        },
        include: {
      ...
      const scoredVendors = getBestMatchingVendors(vendorProfiles, jobContext, 10)

      // Vendor bilgilerini detaylandır
      matchedVendors = scoredVendors.map((scored) => {
