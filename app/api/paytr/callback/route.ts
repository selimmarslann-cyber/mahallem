...
export async function POST(request: NextRequest) {
  try {
    ...
    const failedReasonMessage = formData.get('failed_reason_msg') as string | null

    if (!merchantOid || !status || !paymentAmount) {
      console.error('PayTR callback: Eksik parametreler', { merchantOid, status, paymentAmount })
    ...
    if (status === 'success') {
      ...
      try {
        ...
        console.log('PayTR callback: Ödeme alındı maili gönderildi', { orderId: merchantOid })
      } catch (mailError) {
