...
export default function AccountReferralPage() {
  ...
  const totalInvited = overview.level1Count + overview.level2Count + overview.level3Count + overview.level4Count + overview.level5Count
    ...
    <div className="space-y-6">
      ...
      >
        <div className="bg-gradient-to-br from-amber-500 via-orange-500 to-red-500 text-white rounded-lg p-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">Arkadaşlarını Davet Et, Mahalle Ekonomisini Büyüt</h1>
              <p className="text-white/90 mt-2">
                Arkadaşların sipariş verdikçe, platform komisyonundan <strong>ömür boyu</strong> kazan.
                Downline ciro büyüdükçe rank atla, bonus kazan!
              </p>
            </div>
          </div>
        </div>
          ...
          <CardContent className="space-y-4">
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium">Toplam Network Ciro (GMV)</span>
                <span className="text-lg font-bold">{formatCurrency(overview.currentGMV)}</span>
              </div>
            ...
            {overview.currentRank < 4 && (
              <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <p className="text-xs text-blue-600 mt-1">
                  Rank atladığında bonus: +{RANK_BONUSES[overview.currentRank + 1]}%
                </p>
              </div>
            )}
      ...
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>Kodunu veya linkini paylaş, kazanmaya başla</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <Label className="text-sm font-medium mb-2 block">Referans Kodu</Label>
              <div className="flex gap-2">
                ...
                <Button
                  variant="outline"
                  onClick={() => copyToClipboard(overview.currentReferralCode, 'code')}
                >
                  {copied === 'code' ? (
                    <>
            ...
            <div>
              <Label className="text-sm font-medium mb-2 block">Davet Linki</Label>
              <div className="flex gap-2">
                <Input
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Toplam Referral Kazancı</CardTitle>
              <TrendingUp className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{formatCurrency(overview.totalReferralEarnings)}</div>
        ...
        >
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Toplam Bölge Kazancı</CardTitle>
              <MapPin className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{formatCurrency(overview.totalRegionEarnings)}</div>
              <p className="text-xs text-muted-foreground mt-1">
        ...
        >
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Toplam Davet Edilen</CardTitle>
        ...
        >
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Cüzdan Bakiyen</CardTitle>
              <Gift className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{formatCurrency(overview.currentBalance)}</div>
      ...
      >
        <Card>
          <CardHeader>
            ...
            <CardDescription>Her level'den ne kadar kazandığını gör</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {[1, 2, 3, 4, 5].map((level) => {
                const count = overview[`level${level}Count` as keyof ReferralOverview] as number
                const earnings = overview[`level${level}Earnings` as keyof ReferralOverview] as number
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Referans kodunu veya linkini arkadaşlarınla paylaş. WhatsApp, e-posta veya sosyal medya üzerinden.
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-4">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Arkadaşın ve onun davet ettikleri (L2-L5) sipariş verdikçe sen de kazan. Network ciro büyüdükçe rank atla, bonus kazan!
                  </p>
                </div>
              </div>
            </div>
