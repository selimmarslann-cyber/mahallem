...
import { Star, MessageCircle, MapPin, Clock, User, ArrowLeft, Zap, TrendingUp, DollarSign, FileText } from 'lucide-react'
...
export default function JobDetailPage() {
  ...
  const [priceGuide, setPriceGuide] = useState<{ min: number; max: number; average: number } | null>(null)
  ...
  const getSubServiceName = (categoryId: string, subServiceId: string | null) => {
  ...
  const subServiceName = getSubServiceName(job.mainCategoryId, job.subServiceId)
  
  // Average response time hesapla
  const avgResponseTime = matchedVendors.length > 0
    ? matchedVendors
        .filter((v) => v.avgResponseTime !== null)
        .reduce((sum, v) => sum + (v.avgResponseTime || 0), 0) / matchedVendors.filter((v) => v.avgResponseTime !== null).length
    : null
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-md mx-auto px-4 py-6 space-y-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-gray-900">
                    {job.addressText || `${job.district || ''} ${job.city || ''}`.trim()}
                  </p>
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <p className="text-gray-900">{new Date(job.createdAt).toLocaleDateString('tr-TR')}</p>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
            <CardContent className="p-4">
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <p className="text-2xl font-bold text-blue-600">{matchedVendors.length}</p>
                ...
                {avgResponseTime !== null && (
                  <div>
                    ...
                    <p className="text-xs text-gray-600 mt-1">Ort. Yanıt Süresi</p>
                  </div>
                )}
                {priceGuide && (
                  <div>
                    <p className="text-2xl font-bold text-blue-600">{priceGuide.average.toFixed(0)}₺</p>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-green-800">
                    <div>
                      ...
                      <span className="font-bold">{priceGuide.min.toFixed(0)} ₺</span>
                    </div>
                    <div>
                      <span className="font-medium">Ortalama:</span>{' '}
                      <span className="font-bold">{priceGuide.average.toFixed(0)} ₺</span>
                    </div>
                    <div>
                      <span className="font-medium">Max:</span>{' '}
                      <span className="font-bold">{priceGuide.max.toFixed(0)} ₺</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              {matchedVendors.map((vendor) => (
                <Card key={vendor.businessId} className="hover:shadow-lg transition-shadow">
                  <CardContent className="p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <div className="flex-1 min-w-0">
                            ...
                            {vendor.onlineStatus === 'ONLINE' && (
                              <BadgePremium type="verified" size="sm" className="mt-1" />
                            )}
                          </div>
                        ...
                        <div className="flex flex-wrap gap-3 text-xs text-gray-600 mb-3">
                          ...
                          {vendor.avgResponseTime !== null && (
                            <div className="flex items-center gap-1">
                              ...
                              <span>{vendor.avgResponseTime.toFixed(1)} saat</span>
                            </div>
                          )}
                          {vendor.completionRate !== null && (
                            <div className="flex items-center gap-1">
                              <Zap className="w-3 h-3" />
                              <span>%{vendor.completionRate.toFixed(0)} tamamlanma</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex gap-2 mt-4">
                          <Button
                            ...
                            className="flex-1"
                            onClick={() => {
                              router.push(`/jobs/${job.id}/offers?businessId=${vendor.businessId}`)
                            }}
                          >
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              router.push(`/jobs/${job.id}/chat?businessId=${vendor.businessId}`)
                            }}
                          >
        ...
        {job.offers.length > 0 && (
          <div>
            <h2 className="text-xl font-bold mb-4">Gelen Teklifler ({job.offers.length})</h2>
                ...
                <Card key={offer.id} className="border-2 border-primary/20">
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <div className="flex-1">
                        <h3 className="font-semibold mb-1">{offer.business.name}</h3>
                        <div className="flex items-center gap-2 text-sm text-gray-600">
                          <Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                          <span>{offer.business.avgRating.toFixed(1)}</span>
                          <span>•</span>
                          <span>{offer.business.reviewCount} değerlendirme</span>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-2xl font-bold text-primary">{offer.amount.toFixed(0)} ₺</p>
                        <Badge
                          variant={offer.status === 'ACCEPTED' ? 'default' : 'secondary'}
                          className="mt-1"
                        >
                          {offer.status === 'ACCEPTED' ? 'Kabul Edildi' : 'Bekliyor'}
                        </Badge>
                      </div>
                    </div>
                    {offer.message && (
                    ...
                    <div className="flex gap-2">
                      ...
                      {offer.status === 'PENDING' && (
                        <Button
                          ...
                          className="flex-1"
                          onClick={() => {
                            router.push(`/jobs/${job.id}/chat?businessId=${offer.business.id}`)
                          }}
                        >
                          <MessageCircle className="w-4 h-4 mr-1" />
                          Mesaj
        ...
        {job.acceptedBy && (
          <Card className="border-2 border-green-200 bg-green-50">
            <CardHeader>
              <CardTitle className="text-lg text-green-900">Kabul Eden Esnaf</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-lg mb-1">{job.acceptedBy.name}</h3>
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => {
